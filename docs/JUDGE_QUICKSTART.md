...
If codes are visible, use **Judge**: `smartport2026`. Other role codes are Port Admin `smartport2026`, Fleet Owner `smartport2026`, and Driver `smartport2026`.

Full low-friction path: `/demo-access` → quick-fill Judge code → `/demo-tour` → `/gemini-agent` → `/agent-governance` → `/ops-ingest` → `/execution/plans` → `/fleet/trucks/SPQ-2026-0042` → simulated WhatsApp → `/driver-app` → `/fleet/notifications` → `/enterprise-readiness` → `/health/integrations`.
