...
public class AiAgentService : IAiAgentService
{
    ...
    public async Task<AgentAnswer> AskAsync(string question)
    {
        ...
        if (!string.IsNullOrWhiteSpace(_settings.OpenAiApiKey) && !_settings.FallbackMode)
        {
            try
            {
                return await AskLlmAsync(question, ctx);
            }
            catch
            {
    ...
    private async Task<AgentAnswer> AskLlmAsync(string question, OperationalContext ctx)
    {
        // Build a context summary for the LLM
        var contextSummary = $"""
            Current port operational data (SYNTHETIC DEMO DATA ONLY):
            - Vessels in port: {ctx.VesselsInPort}, delayed: {ctx.VesselsDelayed}, at anchor: {ctx.VesselsAtAnchor}
            - Berth utilisation: {ctx.BerthUtilisationPct}% ({ctx.BerthsOccupied} occupied, {ctx.BerthsAvailable} available)
            - Yard occupancy: {ctx.YardOccupancyPct:F1}%, dwell alerts: {ctx.DwellAlerts}
            - Gate queue: {ctx.TrucksInQueue} trucks, gates operational: {ctx.GatesOperational}
            - Active disruptions: {ctx.ActiveDisruptions} (critical: {ctx.CriticalDisruptions})
            - Load-shedding active: {ctx.LoadSheddingActive}, road congestion: {ctx.RoadCongestionActive}, gate delay: {ctx.GateDelayActive}
            - Active dispatch trips: {ctx.ActiveTrips}, high-risk: {ctx.HighRiskTrips}
            - Idling today: {ctx.TotalIdlingMinutesToday:F0} min, CO2: {ctx.EstimatedCo2Today:F1} kg
            - Open incidents: {ctx.OpenIncidents}, active alerts: {ctx.ActiveAlerts}
            - Top disruptions: {string.Join("; ", ctx.TopDisruptions)}
            
            Question: {question}
            """;

        using var client = _httpClientFactory?.CreateClient() ?? new HttpClient();
        client.DefaultRequestHeaders.Add("Authorization", $"Bearer {_settings.OpenAiApiKey}");

        var payload = new {
            model = _settings.OpenAiModel,
            messages = new[] {
                new { role = "system", content = _settings.SystemPrompt },
                new { role = "user", content = contextSummary }
            },
            max_tokens = 400,
            temperature = 0.3
        };
        ...
        var answerText = result?.Choices?.FirstOrDefault()?.Message?.Content ?? "No response.";
        ...
        return new AgentAnswer {
            ...
            Answer    = answerText,
            IsFromLlm = true,
            DataPoints = new() { "Response generated by LLM with live operational context" }
        };
    }
