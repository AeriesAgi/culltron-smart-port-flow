...
public class SmartPortCopilotChatService : ISmartPortCopilotChatService
{
    ...
    private async Task<CopilotChatResponse> GenerateResponseAsync(string prompt, OperationalContext ctx, string? conversationHistory = null)
    {
        ...
        ApplyEngineStatus(response, usedGemini: false, fallbackReason: "Gemini unavailable — deterministic fallback used");
        return response;
    }
    ...
    {
        response.GeneratedBy = usedGemini ? "Gemini" : string.IsNullOrWhiteSpace(fallbackReason) ? "Deterministic fallback" : "Hybrid fallback";
    ...
    {
        new() { Label = "Generated by", Value = generatedBy, Tone = generatedBy == "Gemini" ? "success" : generatedBy.Contains("Hybrid") ? "warning" : "info" },
    ...
    private CopilotChatResponse BuildDemoSummary(string prompt, OperationalContext ctx) =>
        Base(prompt, "demo", "Medium", 94, "Competition Demo Narrative",
            ...
            "Judges see premium UI, Gemini/deterministic fallback AI, scenario planning, sustainability impact and accountable decision history in under three minutes.",
