...
public enum DataProvenanceType { SyntheticDemoData, ExternalPortFeedPlaceholder, ManualOperatorInput, WhatsAppDriverCheckIn, AndroidDriverApp, WebDriverCompanion, MobileApi, GeminiAiRecommendation, DeterministicFallback, FutureLiveConnector }
public enum DriverEventType { WhatsAppLocationRequested, WhatsAppLocationShared, AndroidAppStatusChecked, DriverAcknowledgedInstruction, DriverConfirmedHolding, DriverArrivedAtStaging, DriverProceedingToGate, DriverArrivedAtGate, DriverCompletedJob, DriverReady, DriverBreak, DriverLunch, DriverDelayed, DriverIssueReported, AndroidLocationCheckIn, WebLocationCheckIn, MobileApiLocationCheckIn }
...
public enum ExecutionPlanStatus { Draft, Approved, Dispatching, InProgress, Completed, Exception }
...
public class DriverEventRequestDto { public string Reference { get; set; } = string.Empty; [JsonConverter(typeof(JsonStringEnumConverter))] public DriverEventType EventType { get; set; } public string SourceLabel { get; set; } = string.Empty; public string Status { get; set; } = string.Empty; public string Action { get; set; } = string.Empty; public decimal? Latitude { get; set; } public decimal? Longitude { get; set; } public decimal? Accuracy { get; set; } public string? LocationLabel { get; set; } public DateTime? Timestamp { get; set; } }
...
public class ExecutionPlanDto { public string PlanId { get; set; } = Guid.NewGuid().ToString("N"); public DateTime CreatedAt { get; set; } = DateTime.UtcNow; public string ScenarioName { get; set; } = "Demo gate congestion execution plan"; public string AffectedFleetOperator { get; set; } = "All demo fleets"; public ExecutionPlanStatus Status { get; set; } = ExecutionPlanStatus.Draft; public string PortStage { get; set; } = "Gate metering / staging coordination"; public string DisruptionSummary { get; set; } = "Gate congestion and berth timing variance require controlled truck release."; public string ModelSource { get; set; } = "Deterministic fallback · Gemini-ready"; public decimal EstimatedDieselLitresSaved { get; set; } public List<string> FleetOwnerInstructions { get; set; } = new(); public List<string> DriverInstructionSet { get; set; } = new(); public List<ExecutionPlanTruckActionDto> TruckActions { get; set; } = new(); public int ExpectedIdlingMinutesAvoided { get; set; } public decimal ExpectedCo2KgAvoided { get; set; } public int ExpectedDelayReductionMinutes { get; set; } public decimal ConfidenceScore { get; set; } public string Explanation { get; set; } = string.Empty; public List<OperationalAuditEntryDto> AuditEntries { get; set; } = new(); }

public sealed class FleetInstructionRequestDto { public string Reference { get; set; } = string.Empty; public string Instruction { get; set; } = string.Empty; }
public sealed class ExecutionPlanActionRequestDto { public string PlanId { get; set; } = string.Empty; }
